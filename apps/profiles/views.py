...
'''
function based view example of AgentListAPIView
----------------------------------------------------------------
@api_view(['GET'])
@permission_classes(['permissions.IsAutheniticated'])
def get_agent_list(request):
    agents = Profile.objects.filter(is_agent=True)
    serializer = ProfileSerializer(agents, many=True)
    name_spaced_response = {'request':serializer.data}
    reutrn Response(name_spaced_response, status=status.HTTP_200_OK)
